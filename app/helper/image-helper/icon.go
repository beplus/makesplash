package image_helper

var IconResized = []ResizeImage{
	// iPhone: Notification (iOS 7 - 10) (@2x, @3x)
	// iPad: Notifications (iOS 7 - 10) (@1x, @2x)
	{
		"/" + Ios + "/", "Icon_20pt@1_20x20", 20, 20,
	},
	{
		"/" + Ios + "/", "Icon_20pt@2_40x40", 40, 40,
	},
	{
		"/" + Ios + "/", "Icon_20pt@3_60x60", 60, 60,
	},
	// iPhone: Spotlight (iOS 5 - 6), Settings (iOS 5 - 10) (@1x, @2x, @3x)
	// iPad: Settings (iOS 5 - 10) (@1x, @2x)
	{
		"/" + Ios + "/", "Icon_29pt@1_29x29", 29, 29,
	},
	{
		"/" + Ios + "/", "Icon_29pt@2_58x58", 58, 58,
	},
	{
		"/" + Ios + "/", "Icon_29pt@3_87x87", 87, 87,
	},
	// iPhone: Spotlight (iOS 7 - 10) (@2x, @3x)
	// iPad: Spotlight (iOS 7 - 10) (@1x, @2x)
	{
		"/" + Ios + "/", "Icon_40pt@1_40x40", 40, 40,
	},
	{
		"/" + Ios + "/", "Icon_40pt@2_80x80", 80, 80,
	},
	{
		"/" + Ios + "/", "Icon_40pt@3_120x120", 120, 120,
	},
	// iPad: Spotlight (iOS 5 - 6)
	{
		"/" + Ios + "/", "Icon_50pt@1_50x50", 50, 50,
	},
	{
		"/" + Ios + "/", "Icon_50pt@2_100x100", 100, 100,
	},
	// iPhone: App (iOS 5 - 6)
	{
		"/" + Ios + "/", "Icon_57pt@1_57x57", 57, 57,
	},
	{
		"/" + Ios + "/", "Icon_57pt@2_114x114", 114, 114,
	},
	// iPhone: App (iOS 7 - 10)
	{
		"/" + Ios + "/", "Icon_60pt@2_120x120", 120, 120,
	},
	{
		"/" + Ios + "/", "Icon_60pt@3_180x180", 180, 180,
	},
	// iPad: App (iOS 5 - 6)
	{
		"/" + Ios + "/", "Icon_72pt@1_72x72", 72, 72,
	},
	{
		"/" + Ios + "/", "Icon_72pt@2_144x144", 144, 144,
	},
	// iPad: App (iOS 7 - 10)
	{
		"/" + Ios + "/", "Icon_76pt@1_76x76", 76, 76,
	},
	{
		"/" + Ios + "/", "Icon_76pt@2_152x152", 152, 152,
	},
	// iPad Pro: App (iOS 9 - 10)
	{
		"/" + Ios + "/", "Icon_83.5pt@2_167x167", 167, 167,
	},
	// AppStore
	{
		"/" + Ios + "/", "AppStore_1024x1024", 1024, 1024,
	},
	// Android
	{
		"/" + Android + "/mipmap-ldpi/", "ic_launcher", 36, 36,
	},
	{
		"/" + Android + "/mipmap-mdpi/", "ic_launcher", 48, 48,
	},
	{
		"/" + Android + "/mipmap-hdpi/", "ic_launcher", 72, 72,
	},
	{
		"/" + Android + "/mipmap-xhdpi/", "ic_launcher", 96, 96,
	},
	{
		"/" + Android + "/mipmap-xxhdpi/", "ic_launcher", 144, 144,
	},
	{
		"/" + Android + "/mipmap-xxxhdpi/", "ic_launcher", 192, 192,
	},
}
